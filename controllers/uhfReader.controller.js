...
exports.getUHFReader = asyncHandler(async (req, res, next) => {
  ...
  if (!reader) {
    return next(new ErrorResponse(`UHF reader not found with id ${req.params.id}`, 404));
  }
...
exports.createUHFReader = asyncHandler(async (req, res, next) => {
  ...
  if (existingReader) {
    return next(new ErrorResponse(`UHF reader with ID ${req.body.uhfId} already exists`, 400));
  }
...
exports.updateUHFReader = asyncHandler(async (req, res, next) => {
  ...
  // Update reader
  reader = await UHFReader.findByIdAndUpdate(
    req.params.id,
    req.body,
    {
      new: true,
...
exports.getUHFReaderByUhfId = asyncHandler(async (req, res, next) => {
  ...
  if (!reader) {
    return next(new ErrorResponse(`UHF reader not found with UHF ID ${req.params.uhfId}`, 404));
  }
